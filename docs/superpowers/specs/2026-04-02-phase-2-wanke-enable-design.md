...
- 万科东第：启用 `anjuke_mobile_sale_search`
...
3. 只为 `万科东第` 补 `anjuke_mobile_sale_search` 适配
...
[
  ...
  },
  {
    ...
    "district": "待确认",
    "status": "active",
    "sourceProvider": "anjuke_mobile_sale_search",
    "sources": {
...
- `active + anjuke_mobile_sale_search`
...
- `anjuke_mobile_sale_search` -> 新的万科东第 collector
- `none` -> 跳过
...
- 走 `anjuke_mobile_sale_search` collector
- collector 负责把该来源的字段归一化成与现有 Fang 路径兼容的 run artifact
- downstream 的 `build-series.ts` 和 `build-weekly-report.ts` 不应因为来源不同而分叉逻辑

#### 6.2.1 来源契约

`anjuke_mobile_sale_search` 的来源契约在二期中固定为：

- 入口 URL：`sources.anjukeSaleSearchUrl`
- 当前只服务 `万科东第`
- parser 只保留能明确匹配 `万科东第` 的挂牌项
...
2. collector / pipeline
   - `sourceProvider = anjuke_mobile_sale_search` 能被正确分发
...
2. 调整 `万科东第` 的 segment
3. 补新来源 fixture / parser / collector
...
- 通过一个新 provider 验证“按小区混合来源”策略可行
